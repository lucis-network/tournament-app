...
export default observer(function HistoryTable({
  ...
  const [isModalContactVisible, setModalContactVisible] =
    useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [loadingList, setLoadingList] = useState<number[]>([]);
  const [listChestUserInfo, setListChestUserInfo] = useState<
    LuckyChestUserInfo | undefined
  >(undefined);
  // const {
  ...
  const getLuckyChestUserInfo = async (
    game_platform_id: Game | undefined,
    tier: LuckyChestTier | undefined,
    page: number,
    limit: number
  ) => {
    setLoading(true);
  ...
  useEffect(() => {
    ...
    userHistory?.map((item, index) => {
      dataSource.push({
        ...
        amount_of_currency: item?.amount_of_currency,
        number_of_prize: item?.number_of_prize,
      });
    });
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (text: string, data: any, index) => {
        return (
          <div className={s.prizeWrap} key={index}>
            <div className={`${s.prize} ${data.prize.rarity}`}>
              <PrizePopover
                image={data.prize.img}
                title={getPrizeTitle(data)}
            ...
            <ButtonClaim
              ...
              onClick={() =>
                handleClaimChestPrize(
                  index,
                  data.prize?.category.currency_uid,
                  data.prize?.category?.currency_type,
                  data.user_prize_history_uid.toString(),
                  data.code
                )
              }
              disabled={
                data.is_claimed ||
                claimingChestPrize === data.user_prize_history_uid
              }
              buttonText={
                !data.prize?.category?.currency_type ? "Archived" : null
              }
  ...
  function getPrizeTitle(item: UserHistory) {
    if (!item) {
      return "";
    }
          ...
          if (code !== "UnAuth") {
            switch (code) {
              case ClaimChestPrizeErrorCode.UserHistoryNotFound:
                antMessage.error(
                  "Invalid prize code. Please contact Lucis Support for more details."
                );
                break;
  ...
  return (
    <>
      ...
      <PopupContactRaffles
        ...
        description={
          <div>
            Congratulations on your lucky win from Lucis.
            <br />
            Your prize code is{" "}
            <b
              style={{ color: "#00F9FF", cursor: "pointer" }}
              onClick={handlePrizeCodeClick}
            >
              {prizeCode}
            </b>
            . Please contact and send the code to Lucis Supporter to receive the
            prize.
          </div>
        }
      />
