...
const DailyMission = () => {
  ...
  const [getDailyMission, stateDailyMissionFetch] = useMutation(
    GET_OR_SET_DAILY_MISSION,
    {
      variables: {
        game_uid: "03",
  ...
  const handleUpdateMissions = () => {
    updateDailyMission().then((response) => {
      ...
      <div className={s.recentMatchesWrap}>
        ...
        <div className={s.recentMatchesList}>
            ...
            FETCH_API.slice(0, lengthShowMore).map((item) => {
              return (
                <div className={s.recentMatchesItem} key={`${item?.id}`}>
                  <div className={s.recentMatchesLogo}>
                    <Image
                      src="/assets/P2E/csgo-logo-icon.png"
                      preview={false}
                      alt=""
                    />
                  </div>
                  <div className={s.recentMatchReward}>1000 Lucis point</div>
                  <div className={s.recentMatchReward}>100 Lucis token</div>
                  <div className={s.recentMatchScore}>{item?.slug}</div>
                  <div className={s.recentMatchActions}>
                    <Button loading={loading} onClick={loadingClaim}>
                      Claim
                    </Button>
                  </div>
                </div>
          ...
          {FETCH_API.length > 5 && lengthShowMore < FETCH_API.length && (
            <div className={s.recentMatchesBtn} onClick={loadMore}>
