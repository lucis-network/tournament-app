...
const MissionList = ({
  ...
  const listMissionDone = missions?.map((item) => {
    ...
    const currentPercent =
      ((achieved as number) / (item?.mission?.goal as unknown as number)) * 100;

    return currentPercent >= 100;
  });

  const lengthMissionDone =
    listMissionDone?.filter((item) => item)?.length ?? 0;
  ...
  return (
    <div className={s.dailyMissionsList}>
              ...
              return (
                <div className={s.checkListMissionItem} key={index}>
                  {currentGame === Game.LOL ? (
                    <img
                      className={s.csgoImage}
                      src="/assets/P2E/lol-game.svg"
                      alt="lol-checklist-mission"
                    />
                  ) : (
                    <img
                      className={s.csgoImage}
                      src="/assets/P2E/csgo/csgo-checklist-mission.png"
      ...
      <div className={s.subHeader}>
        <p>{title ?? "Completed all the daily misions to receive"}</p>
        <div className={s.updateButton} onClick={() => clickUpdateButton()}>
          <img
            src="/assets/P2E/reload-icon.png"
            alt=""
            ...
            return (
              <MissionItem
                ...
                handleUpdateMission={async () =>
                  await handleUpdateMissions(false, false)
                }
