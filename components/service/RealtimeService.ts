...
import apoloClient, {clientAdmin, clientP2E} from "utils/apollo_client";
export class RealtimeService {
  ...
  public async subscriptionAdmin() {
      const response = await clientAdmin.subscribe<{ pushNotification: any }>({
          query: gql`
              subscription ($user_id: Int!){
                  pushNotification(user_id: $user_id) {
                      new_noti {
